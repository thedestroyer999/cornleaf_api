// server.js (Versi Perbaikan untuk PostgreSQL & Vercel)
require('dotenv').config();
const express = require('express');
const { Pool } = require('pg'); // PERUBAHAN: Menggunakan 'pg' untuk PostgreSQL
...
// --- Koneksi Database (Versi PostgreSQL) ---
const db = new Pool({
...
// Fungsi untuk tes koneksi saat aplikasi pertama kali jalan
...
// --- Rute API (Direfaktor untuk PostgreSQL) ---
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        // PERUBAHAN: Sintaks query dan cara ambil hasil
        const { rows: users } = await db.query('SELECT email FROM users WHERE email = $1', [email]);
        if (users.length > 0) {
            return res.status(400).json({ message: 'Email ini sudah terdaftar.' });
        }
        const hashedPassword = await bcrypt.hash(password, 10);
        await db.query('INSERT INTO users (full_name, email, password) VALUES ($1, $2, $3)', [fullName, email, hashedPassword]);
...
app.post('/api/forgot-password', async (req, res) => {
    try {
        ...
        await db.query('UPDATE users SET reset_token = $1, reset_token_expiry = $2 WHERE id = $3', [resetToken, expiryDate, user.id]);
        
        // Ganti localhost:3000 dengan URL frontend Anda
...
app.post('/api/reset-password', async (req, res) => {
    try {
        ...
        const { rows: users } = await db.query('SELECT * FROM users WHERE reset_token = $1 AND reset_token_expiry > NOW()', [token]);
        if (users.length === 0) {
...
app.post('/api/history/save', verifyToken, async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO scan_history (user_id, image_data, detection_result, accuracy, recommendation) VALUES ($1, $2, $3, $4, $5)', [req.user.id, image_data, detection_result, accuracy, JSON.stringify(recommendation)]);
...
app.get('/api/history', verifyToken, async (req, res) => {
    try {
        const { rows: history } = await db.query('SELECT id, image_data, detection_result, accuracy, scanned_at FROM scan_history WHERE user_id = $1 ORDER BY scanned_at DESC', [req.user.id]);
        res.json(history);
    } catch (error) {
...
app.delete('/api/history/:id', verifyToken, async (req, res) => {
    try {
        const { rowCount } = await db.query('DELETE FROM scan_history WHERE id = $1 AND user_id = $2', [req.params.id, req.user.id]);
        if (rowCount === 0) return res.status(404).json({ message: 'Riwayat tidak ditemukan.' });
...
app.get('/api/profile', verifyToken, async (req, res) => {
    try {
        const { rows } = await db.query('SELECT id, full_name, email, profile_picture FROM users WHERE id = $1', [req.user.id]);
...
app.put('/api/profile/details', verifyToken, async (req, res) => {
    try {
        ...
        await db.query('UPDATE users SET full_name = $1, profile_picture = $2 WHERE id = $3', [fullName, profilePicture, req.user.id]);
        
        const { rows } = await db.query('SELECT id, full_name, email FROM users WHERE id = $1', [req.user.id]);
        const user = rows[0];
        const payload = { user: { id: user.id, email: user.email, fullName: user.full_name } };
        const token = jwt.sign(payload, JWT_SECRET, { expiresIn: '1h' });
...
app.put('/api/profile/password', verifyToken, async (req, res) => {
    try {
        ...
        await db.query('UPDATE users SET password = $1 WHERE id = $2', [newHashedPassword, req.user.id]);
...
app.get('/api/stats', verifyToken, async (req, res) => {
    try {
        ...
        const queries = [
            db.query('SELECT COUNT(*) as "totalScans" FROM scan_history WHERE user_id = $1', [userId]),
            db.query("SELECT COUNT(*) as \"diseasesDetected\" FROM scan_history WHERE user_id = $1 AND detection_result != 'Sehat'", [userId]),
            db.query('SELECT AVG(accuracy) as "averageAccuracy" FROM scan_history WHERE user_id = $1', [userId]),
            db.query('SELECT COUNT(*) as "scansThisMonth" FROM scan_history WHERE user_id = $1 AND EXTRACT(MONTH FROM scanned_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM scanned_at) = EXTRACT(YEAR FROM CURRENT_DATE)', [userId])
        ];
        
        const results = await Promise.all(queries);

        res.json({
            totalScans: parseInt(results[0].rows[0].totalScans) || 0,
            diseasesDetected: parseInt(results[1].rows[0].diseasesDetected) || 0,
            averageAccuracy: parseFloat(results[2].rows[0].averageAccuracy) || 0,
            scansThisMonth: parseInt(results[3].rows[0].scansThisMonth) || 0
        });
    } catch (error) {
...
// --- Start the Server ---
// Di Vercel, kita tidak perlu app.listen(), karena Vercel menanganinya
// app.listen(PORT, () => {
//   console.log(`Backend API is running on http://localhost:${PORT}`);
// });

// PERUBAHAN: Ekspor aplikasi untuk digunakan Vercel
module.exports = app;